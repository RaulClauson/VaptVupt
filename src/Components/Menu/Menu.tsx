...
const Menu = () => {
  ...
  const menu_hide = () => {
    ...
    document
      .getElementById("sombra_menu")
      ?.classList.toggle("sombra_menu_open");
  };
  ...
  return (
    <>
      ...
      <header id="header" role="navigation">
        <nav>
          <div>
            <button onClick={menu_hide} tabIndex={1}>
            ...
            <div className="links">
              ...
              >
                <Logo_icon width="17px" height="15px" />
                <p>Diagnóstico</p>
              </Link>
              <Link
                to="/Oficinas"
                tabIndex={1}
                className={activeLink === "/Oficinas" ? "ativo" : ""}
                onClick={() => {
                  handleLinkClick("/");
                  if (window.matchMedia("(max-width: 888px)").matches) {
                    menu_hide();
              ...
              >
                <HiMiniWrenchScrewdriver size={17} />
                <p>Oficinas</p>
              </Link>
              <Link
                to="/MeuCarro"
                tabIndex={1}
                className={activeLink === "/MeuCarro" ? "ativo" : ""}
                onClick={() => {
                  handleLinkClick("/");
                  if (window.matchMedia("(max-width: 888px)").matches) {
                    menu_hide();
          ...
          <ul>
            <Link
              to="/Ajuda"
              tabIndex={1}
              className={activeLink === "/Ajuda" ? "ativo" : ""}
              onClick={() => {
                handleLinkClick("/");
                if (window.matchMedia("(max-width: 888px)").matches) {
                  menu_hide();
            ...
            <button type="button" onClick={acessibilidade} tabIndex={1}>
              <BiUniversalAccess size={22} />
              <p>Acessibilidade</p>
            </button>
            <button type="button" onClick={toggleDarkMode} tabIndex={1}>
            ...
            <Link
              to="/Configuracoes"
              tabIndex={1}
              className={activeLink === "/Configuracoes" ? "ativo" : ""}
              onClick={() => {
                handleLinkClick("/");
                if (window.matchMedia("(max-width: 888px)").matches) {
                  menu_hide();
        ...
        <Link
          to="/Perfil"
          tabIndex={1}
          className={activeLink === "/Perfil" ? "ativo" : ""}
          onClick={() => {
            handleLinkClick("/");
            if (window.matchMedia("(max-width: 888px)").matches) {
              menu_hide();
